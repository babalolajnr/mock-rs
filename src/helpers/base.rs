...
pub fn random_index<T>(arr: &[T]) -> usize {
    let random_index = thread_rng().gen_range(0..arr.len());
    random_index
}

/// Returns a random element an array/vector
pub fn random_element<'a, T>(arr: &'a Vec<&T>) -> &'a T
where
    T: ?Sized,
{
    let random_index = random_index(&arr);
    &arr[random_index]
}

/// Replaces all occurences of '#'s with a random number
pub fn numerify(string: Option<&str>) -> String {
...
pub fn bothify(string: Option<&str>) -> String {
    let string = string.unwrap_or("## ??");
    ...
    let random_string = string
        ...
        .map(|i| {
            if i == '#' {
                ...
                let random_set = random_element(&charset);
...
mod tests {
    ...
    fn numerify_works() {
        ...
        let result = numerify(Some(string));
    ...
    fn test_bothify() {
        let string = "?####";
        let result = bothify(Some(string));
        assert_eq!(result.len(), string.len());
    }
    ...
    fn random_key_for_str_slice() {
        ...
        let result = random_key(&hash_map);
    ...
    fn get_random_letter() {
        let random_letter = random_letter();
